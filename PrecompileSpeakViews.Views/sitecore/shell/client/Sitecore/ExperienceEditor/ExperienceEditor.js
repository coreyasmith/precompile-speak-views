    ...
    function(Sitecore, ExperienceEditorContext, ExperienceEditorProxy, TranslationUtil) {
        ...
        var experienceEditor = {
                ...
                experienceEditor.modifiedHandling(null, function(isOk) {
                    if (allowParameterRedirection
                        ...
                        != escape(context.itemId)) {
                            var url = "/?sc_itemid=" + context.itemId +
                                "&sc_lang=" + context.language +
                                "&sc_db=" + context.database +
                                "&sc_device=" + context.deviceId +
                                "&sc_mode=" + context.webEditMode;
            ...
            getCurrentTabId: function () {
                var currentTabId = experienceEditor.Common.getCookieValue("sitecore_webedit_activestrip");
                var previewTabId = "VersionStrip_ribbon_tab";
                if (experienceEditor.Web.getUrlQueryStringValue("mode") === "preview" &&
                    document.getElementById(previewTabId)) {
                    return previewTabId;
                }

                var debugTabId = "DebugStrip_ribbon_tab";
                if (experienceEditor.Web.getUrlQueryStringValue("sc_debug") === "1"
                    && document.getElementById(debugTabId)) {
                    return debugTabId;
                }

                var homeTabId = "PageEditorStrip_ribbon_tab";
                if (experienceEditor.Web.getUrlQueryStringValue("mode") === "edit"
                    && currentTabId === debugTabId
                    && document.getElementById(homeTabId)) {
                    return homeTabId;
                }
        ...
        experienceEditor.Web = {
            ...
            postServerRequest: function (requestType, commandContext, handler, async) {
                var urlRequestType = async != undefined ? "async" : "";
                var token = experienceEditor.Common.getAntiForgeryToken();
                jQuery.ajax({
                    url: "/-/speak/request/v1/expeditor" + urlRequestType +"/" + requestType,
                    data: {
